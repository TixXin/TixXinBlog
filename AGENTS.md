...
- 日常开发样本长期保留，隔离自动化测试数据用后清理；两者的数据库、媒体目录及清理范围必须明确分开。样本保留内部稳定标识，支持重复执行与定向清理；开发用途说明仅放在种子脚本注释或维护文档中。
- 博客测试数据使用自然、真实的内容，不在标题、正文、摘要、标签等展示字段中添加“开发示例”“测试数据”“演示用途”等说明文字。
