...
- 本阶段7项模块：已实现数据检查与目录1项，增量样本1项进行中，留言API、前台、后台、维护、完整验收5项待实现。
- 当前完成率14%，只对应本阶段；最终数据、业务和三浏览器验收未完成前不能标记完成。
...
- [~] 开发数据检查、增量样本归属与留言板真实业务；保留用户开发数据库中的长期样本。

## 待处理

以下为下一阶段候选，不计入本阶段7项完成率，需按实际业务目标确定顺序：

- [ ] 开发测试数据保障：各功能配套样本、统一检查/增量补齐、日常库与隔离库分离；不再交付无数据的开发页面。完整范围见 docs/next-goal-development-data-guestbook.md。
- [ ] 留言持久化、审核和后台管理。
- [ ] 项目、图库、友链按业务域逐步接入真实维护。
- [ ] 整理真实运营内容与站点资料；推送、部署及对外发布需对应授权。
