/** @file useLinks.ts @description 友链 SSR 列表与统计，统一 URL 查询并保留上次成功读取的数据 */
import type { LinkPage, LinkQuery } from '~/features/link/types'
...
export async function useLinks() {
  ...
  let initialQuery: LinkQuery = linkQuery(route.query)
  const serverKey = (app.payload.data['links-feed'] as { key?: string } | undefined)?.key
  if (hydrating.value && serverKey) {
    try {
      initialQuery = linkQuery(JSON.parse(serverKey))
    } catch {
      /* 无有效 SSR 查询时沿用初始路由。 */
    }
  }
  // 历史导航可能早于异步页面水合完成；首帧必须与 SSR payload/DOM 使用同一个查询。
  const query = computed(() => (hydrating.value ? initialQuery : linkQuery(route.query))),
    key = computed(() => JSON.stringify(query.value))
  const accepted = ref<{ key: string; page: LinkPage } | null>(null)
  onMounted(() => {
    ready.value = true
    hydrating.value = false
  })
  const feed = useAsyncData('links-feed', async (_app, { signal }) => {
  ...
  const metadata = useAsyncData('links-metadata', (_app, { signal }) =>
  ...
  function changeQuery(patch: Record<string, string | number | undefined>) {
    return router.push({ path: '/links', query: { ...route.query, page: undefined, ...patch } })
  }
  ...
  return {
    query,
    ready,
    links: computed(() => accepted.value?.page.items ?? []),
