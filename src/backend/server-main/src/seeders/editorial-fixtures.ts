/** @file editorial-fixtures.ts @description 独立登记的事实内容候选；只创建草稿，不自动发布或覆盖已有编辑 */
...
export const EDITORIAL_FIXTURE_COUNT = editorialArticles.length + 1
    ...
    async () =>
      (
        await new ProjectService(em).save(null, {
          ...
          sortOrder: 0,
          tags: [
            { label: 'Nuxt', color: 'emerald' },
            { label: 'NestJS', color: 'rose' },
