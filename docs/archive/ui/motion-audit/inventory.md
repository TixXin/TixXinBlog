...
> 历史归档：原文来自 `docs/motion-audit/inventory.md`，保留当时的设计、实施与验收事实。文中的阶段、版本、数量和“当前／下一步”不代表现状；现行入口见[文档导航](../../../README.md)。本机验收产物仍按原仓库相对路径留存。
...
- 历史位置/入口：`src/frontend/web-blog/app/components/about/SkillBars.vue`（组件现已移除，以下保留当时审查事实）。
...
实现：历史源码 `src/frontend/web-blog/app/components/about/SkillBars.vue:83`（组件现已移除）。
      ...
      <div v-if="commentOpen" class="fnc__comments">
        <div v-if="note.comments.length > 0" class="fnc__comment-list">
          <div v-for="c in note.comments" :key="c.id" class="fnc__comment">
            <img :src="../../../motion-audit/c.authorAvatar" :alt="c.authorName" class="fnc__comment
```
...
<Transition name="user-card-fade">
        ...
        >
          <img
            ...
            :src="../../../motion-audit/message.avatar"
```
...
实现：历史源码 `src/frontend/web-blog/app/composables/useMomentPagination.ts:3`（文件现已移除）。

JS 调度/观察器历史行号（对应上方已移除文件）：第 3 行 `* @description 朋友圈无限滚动分页逻辑，含话题/日期筛选、fuse.js 全文搜索与 IntersectionObserver`；第 62 行 `let spinnerTimer: ReturnType<typeof setTimeout> | null = null`；第 88 行 `spinnerTimer = setTimeout(() => {`；第 92 行 `requestAnimationFrame(() => {`；第 100 行 `// IntersectionObserver`；第 101 行 `let observer: IntersectionObserver | null = null`；第 106 行 `observer = new IntersectionObserver(`。
