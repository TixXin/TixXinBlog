...
> 历史归档：原文来自 `docs/backend/api.md`，保留当时的设计、实施与验收事实。文中的阶段、版本、数量和“当前／下一步”不代表现状；现行入口见[文档导航](../../README.md)。本机验收产物仍按原仓库相对路径留存。

本文保留早期 API 设计和后续维护增量记录，其中部分端点尚未实现或已被正式业务契约替代。当前能力以[实际能力清单](../../capability-map.md)及各领域使用文档为准；不能将此处的设计表视为全部已经开放的 HTTP 或 WebSocket 接口。图库正式契约见[图库业务](../../backend/gallery.md)，留言和朋友圈分别见[留言](../../backend/guestbook.md)及[朋友圈](../../backend/moments.md)。
...
图库以稳定作品编号关联受管媒体，公开与管理 DTO 分离。正式契约、字段边界和维护规则见 [图库业务](../../backend/gallery.md)。
...
字段、地址语义、Logo与重复处理见[友链维护](../../backend/links.md)。本轮不提供公开申请、审核队列、邮件、抓取或自动探活。
...
正式字段、统计口径和维护规则见 [项目业务](../../backend/projects.md)。项目进展progress与站点发布status独立，不采集无来源Star/Fork。
...
```json
{
  "profile": {
    ...
    "avatar": "https://...",
    "bio": "...",
    "socials": [{ "icon": "lucide:github", "label": "GitHub", "href": "...", "primary": true }]
  },
