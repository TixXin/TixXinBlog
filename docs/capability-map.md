...
| 图库       | `/gallery`；真实API与PostgreSQL，支持受管图片和外部URL，无Mock回退                                   | 来源切换、预览重试、输入恢复、发布/撤回、排序/删除、器材；v7内容包兼容v1–v6及完整恢复  | 日常22作品（18受管、4外链），公开18；本轮142请求、维护/数据/生产恢复和相关三浏览器243项通过；见[本轮验收](gallery-external-admin-verification.md)                                     |
...
| 管理工作区 | `/admin`及15项管理入口                                                                               | 桌面主内容/导航独立滚动、移动抽屉、共用操作区与列表返回；原权限及输入保护              | 三主题、320/390/760/761px、键盘与焦点、长表单及代表页通过；实际系统缩放/真机等边界见[后台说明](admin-workspace-improvements.md)                                                       |
