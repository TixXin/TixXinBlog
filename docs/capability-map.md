...
首发准备修复了此前遗漏的朋友圈 RSS `/moments.xml` 与公开 JSON `/api/moments.json`：现在读取真实公开 API，按最新时间取30条，撤回及时且上游失败返回503，不再读取旧样本。运行时站点地址与索引规则、干净检出准备目录、受保护首发内容子包及仓库归档见 [首发准备与仓库维护](first-release-readiness.md)。实际生产启用状态以该入口为准。
