...
| `PATCH /admin/moments/:id/comments/:commentId` | 切换 `published/pending/hidden` 审核状态 |
...
隔离集成脚本 `tests/moment-integration.mjs` 已验证 75 个请求，包括未授权访问、非法输入、并发发布/评论去重、分页/筛选、私有数据过滤、目标点赞状态、审核、真实博主回复、图片说明、引用释放、删除重试、审计及迁移后的零结构漂移。该脚本纳入后端 `test:integration`。

本阶段前台、后台页面、开发数据脚本和内容包兼容仍按 `docs/next-stage.md` 推进，不能仅据 API 验证将整轮目标标记为完成。
