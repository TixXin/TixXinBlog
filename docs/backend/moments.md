...
隔离集成脚本 `tests/moment-integration.mjs` 已验证 85 个请求，包括未授权访问、非法输入、并发发布/评论去重、分页/筛选、私有数据过滤、目标点赞状态、审核冲突、提交查询、前后导航、真实博主回复、图片说明、引用释放、删除重试、审计及迁移后的零结构漂移。该脚本纳入后端 `test:integration`。
